...
做法:**開一個新的 session**(不要用 `/clear`)。原因很關鍵——你等下會需要回到「寫文件那一輪」
去修(見下一段),而 `/clear` 會把那輪對話脈絡清掉,新 session 則讓它留著。冷讀者用哪個模型不
重要,重點是它**沒有你們剛剛的對話記憶**。
...
這是一份「功能面」需求文件——技術選型、架構、API 設計刻意留到下一階段,
所以「缺技術細節」不算缺口,請不要當問題提。
請只就「功能需求清不清楚、能不能據此動工」告訴我:
- 哪裡語意含糊、可以有多種解讀?
- 哪裡功能有缺口、我還得回去問才能動?
...
把冷讀者列的問題,帶**回原本那個寫文件的 session**(它還留著完整對話脈絡——這正是為什麼用新
session 冷讀、而不是 `/clear`)。分兩類處理:

- **含糊 / 矛盾**:脈絡裡通常有答案,請它據此把文件改清楚。
- **缺口 / 無法驗證**:這是真的還沒決定——你得補一個決定(必要時對那一塊再 `/grill-me` 補問),
  再更新文件。

改完 → 回到冷讀(另開一個新 session,或叫冷讀者重讀更新後的文件)→ 重複到一個沒有記憶的讀者
也能照著動工。**確認 OK 後,才 `/clear` 進入 02。**
